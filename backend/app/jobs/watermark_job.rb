...
class WatermarkJob < ApplicationJob
  ...
  def perform(filename:, visit_id:, content_type: nil)
  ...
  def watermark_video(filename, content_type)
    ...
    begin
      ...
      ffmpeg_cmd = [
        ...
        "-filter_complex", "[1:v]scale=iw/2.5:-1,format=rgba,colorchannelmixer=aa=0.35,transpose=1[wm];[0:v][wm]overlay=0:0:format=auto,format=yuv420p",
        "-c:a", "copy",
        "-y",
        temp_output.path
      ]
