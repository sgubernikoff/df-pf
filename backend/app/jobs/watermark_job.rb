  ...
  def watermark_video(filename, content_type)
    ...
    begin
      ...
      ffmpeg_cmd = [
         ...
         "-filter_complex", "[1:v]scale=iw/2.5:-1,format=rgba,colorchannelmixer=aa=1.5[wm],transpose=1[wm];[0:v][wm]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2,format=yuv420p",
        "-c:a", "copy",
        "-y",
        temp_output.path
        ]
